"use client"; // Add this line to specify this component is a Client Component
...
export default function SignUpForm() {
    ...
    const [username, setUsername] = useState(""); // Ensure this is an empty string
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const userId = await createAccount(data); // Ensure this function is correctly implemented
            ...
            if (error instanceof Error) {
                setErrorMessage(error.message); // Set error message to show to user
            } else {
    ...
    return (
        <div className="flex-1 sm:p-12 py-8 p-2 flex flex-col overflow-y-auto">
            <Link href={'/'} className="sm:p-0 p-3 w-fit">
                <Image priority src={"/full-logo.svg"} alt="logo" height={150} width={100} className="w-[7.05rem]" />
            </Link>
            <section className="mx-auto py-10 w-full sm:w-5/6 md:w-3/4 lg:w-2/3 xl:w-1/2 flex-1 flex flex-col justify-center">
                ...
                <form className="py-8 sm:py-12 flex flex-col gap-4 sm:gap-6 w-full" onSubmit={createAccountHandler}>
                    ...
                    <div className={`flex items-center py-2 sm:py-3 px-2 sm:px-6 rounded-md myInput ${hasError.email === 1 ? "hasError" : hasError.email === 2 ? "good" : ""} bg-black bg-opacity-5 text-base sm:text-lg w-full`}>
                        ...
                        <input
                            type="email"
                            placeholder="J E M S I T@gmail.com"
                            className="outline-none border-none bg-transparent ml-1 py-3 flex-1 text-sm sm:text-base"
                            value={email || ""} // Ensure this is never null
                    ...
                    <div className={`flex items-center py-2 sm:py-3 px-2 sm:px-6 rounded-md myInput ${hasError.password === 1 ? "hasError" : hasError.password === 2 ? "good" : ""} bg-black bg-opacity-5 text-base sm:text-lg w-full`}>
                        <label className="opacity-40">Password</label>
                        <input
                            type={seePassword ? "password" : "text"}
                            placeholder="******"
                            className="outline-none border-none bg-transparent ml-1 py-3 flex-1 text-sm sm:text-base"
                            value={password || ""} // Ensure this is never null
                            onChange={(e) => setPassword(e.target.value)}
                            required
                        />
                        {seePassword ? (
                            <FaEye className="text-sm cursor-pointer" onClick={() => setSeePassword(!seePassword)} />
                        ) : (
                            <FaEyeSlash className="text-sm cursor-pointer" onClick={() => setSeePassword(!seePassword)} />
                        )}
                        {hasError.password === 1 ? (
                            <FaX className="text-red-500 text-sm cursor-pointer" onClick={() => setPassword("")} />
                        ) : hasError.password === 2 ? (
                            <FaCheck className="text-themeGreen cursor-pointer" />
                        ) : ""}
                    </div>
                    ...
                    <button type="submit" className={
                        ...
                        {isLoading && <Image src={"/gif/loading.gif"} width={25} height={25} alt="loading" className=" mix-blend-screen" />}
                    </button>
                </form>
                <p className="text-center text-base sm:text-lg">
                    Already have an account?{" "}
                    <Link href="/login" className="text-themeBlue font-semibold">
                        Login
                    </Link>
                </p>
            </section>
