...
export async function sendResetUrl(payload) {
    ...
    try {
        const dbRef = collection(db, "accountsReset");
        const collectionSnap = await getDocs(collection(db, "accounts"));

        const cleanEmail = realEscapeString(email);
        const cleanUID = realEscapeString(uid);
        let userId 

        const resetUrl = `https://links.icart.tn/reset-password/${cleanUID}`;
